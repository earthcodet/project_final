...
// address owner
function odistrictSelect(amphurId) {
    ownDistrict = []

    for (let i = 0; i < district.length; i++) {
        if (district[i].AMPHUR_ID == amphurId && district[i].PROVINCE_ID == oProviceId) {
            ownDistrict.push(district[i])
        }
    }
    removeAlloption('ownerSubdistrict')
    for (let i = 0; i < ownDistrict.length; i++) {
        var select = document.getElementById("ownerSubdistrict");
        var option = document.createElement("option");
        option.text = ownDistrict[i].DISTRICT_NAME;
        option.value = ownDistrict[i].DISTRICT_ID;
        select.add(option);
    }

}
function oamphurSelect(proviceId) {
    ownAmphur = []
    oProviceId = proviceId
    for (let i = 0; i < amphur.length; i++) {
        if (amphur[i].PROVINCE_ID == proviceId) {
            ownAmphur.push(amphur[i])
        }
    }
    removeAlloption('ownerDistrict')
    for (let i = 0; i < ownAmphur.length; i++) {
        var select = document.getElementById("ownerDistrict");
        var option = document.createElement("option");
        option.text = ownAmphur[i].AMPHUR_NAME;
        option.value = ownAmphur[i].AMPHUR_ID;
        select.onchange = function () { odistrictSelect(select.value) };
        select.add(option);
    }

}
function ochageProvice(proviceId) {
    oamphurSelect(proviceId)
    odistrictSelect(ownAmphur[0].AMPHUR_ID)
}
function ocreateSelectProvice(data) {
    for (let i = 0; i < data.length; i++) {
        ...
        select.onchange = function () { ochageProvice(select.value) };
        select.add(option);
    }
}
