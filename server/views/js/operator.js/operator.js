...
function setDataUI(data) {
  ...
  document.getElementById("homeId").value =
    data.AID.ADDRESS_HOME_NUMBER === "" ? "-" : data.AID.ADDRESS_HOME_NUMBER;
  document.getElementById("moo").value =
    data.AID.ADDRESS_MOO === undefined ? "-" : data.AID.ADDRESS_MOO;
  document.getElementById("trxk").value =
    data.AID.ADDRESS_TRXK === undefined ? "-" : data.AID.ADDRESS_TRXK;
  document.getElementById("sxy").value =
    data.AID.ADDRESS_SXY === undefined ? "-" : data.AID.ADDRESS_SXY;
  document.getElementById("building").value =
    data.AID.ADDRESS_BUILDING === undefined ? "-" : data.AID.ADDRESS_BUILDING;
  document.getElementById("road").value =
    data.AID.ADDRESS_ROAD === undefined ? "-" : data.AID.ADDRESS_ROAD;
  //ค่าที่ส่งกลับมาอาจเป็น text ต้องการที่เป็น int
  document.getElementById(`province`).value = parseInt(
    getProviceIdByName(data.AID.PROVINCE_NAME)
  );
  document.getElementById(`district`).value = parseInt(
    getAmphureIdByName(data.AID.AMPHUR_NAME)
  );
  document.getElementById(`subdistrict`).value = parseInt(
    getDistrictIdByName(data.AID.DISTRICT_NAME)
  );
  //prsonal
  document.getElementById("title").value =
    data.PERSONAL_TITLE === undefined ? "" : data.PERSONAL_TITLE;
  ...
  document.getElementById("surnameUser").value =
    data.PERSONAL_SURNAME === undefined ? "" : data.PERSONAL_SURNAME;
  document.getElementById("nationality").value =
    data.PERSONAL_NATIONALITY === undefined ? "" : data.PERSONAL_NATIONALITY;
  document.getElementById("race").value =
    data.PERSONAL_RACE === undefined ? "" : data.PERSONAL_RACE;
  document.getElementById("datepicker3").value =
    data.PERSONAL_BIRTHDAY === undefined
      ? "00-00-0000"
  ...
  document.getElementById("datepicker2").value =
    data.PERSONAL_CARD_EXPIRE === undefined
      ? "00-00-0000"
      : data.PERSONAL_CARD_EXPIRE;
  document.getElementById("phone").value = data.PERSONAL_PHONE;
  document.getElementById("fax").value =
    data.PERSONAL_FAX === undefined ? "-" : data.FAX;
  document.getElementById("last-update").value = data.PERSONAL_UPDATE;
}
function preInsert() {
  if (
    document.getElementById("id").value.trim() != "" &&
    document.getElementById("id").value.trim().length === 13
  ) {
    console.log(_isUsed);
    if (_isUsed === false) {
      //address
...
function changeInputBytype(value) {
  ...
  if (value === "นิติบุคคล") {
    ...
    document.getElementById("raceTopic").style.display = "none";
    document.getElementById("nationalityTopic").style.display = "none";
    document.getElementById("dateExpTopic").style.display = "none";
    document.getElementById("datepicker2").style.display = "none";
    document.getElementById("dateBTopic").style.display = "none";
    document.getElementById("datepicker3").style.display = "none";
    document.getElementById("phoneTopic").style.marginLeft = "3.6vw";
    document.getElementById("imgBoxUpload").style.display = "none";
  } else {
